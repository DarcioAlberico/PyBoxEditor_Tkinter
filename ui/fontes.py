...
# ----------------------------------------------------------------------
# Uma fonte do EPUB ao alcance do Tk (ED-08)
# ----------------------------------------------------------------------

_arquivos_registrados: Dict[str, str] = {}
...
def registrar_arquivo(caminho: str) -> Optional[str]:
    """
    Põe uma fonte de arquivo (a fonte embutida num EPUB, extraída para o disco) ao alcance
    do Tk neste processo (`AddFontResourceExW` com `FR_PRIVATE`, como as empacotadas) e
    devolve a **família**, que passa a existir em `tkinter.font.families()` (AC-ED08-9).
    `None` quando o arquivo não é fonte ou o registro falha; fora do Windows, o nome da
    família sem registro (o fontconfig não aceita registro por processo).
    """
    caminho = os.path.abspath(caminho)
    if caminho in _arquivos_registrados:
        return _arquivos_registrados[caminho]
    familia = familia_do_arquivo(caminho)
    if familia is None:
        return None
    if sys.platform.startswith("win"):
        try:
            n = ctypes.windll.gdi32.AddFontResourceExW(ctypes.c_wchar_p(caminho), FR_PRIVATE, 0)
        except Exception:
            n = 0
        if not n:
            return None
    _arquivos_registrados[caminho] = familia
    return familia
